...
/**@memberOf application.handlers.CommonHandler */
		...
		//Reusable method: Get SiteId of the current Asset
		_getWorkorderSiteId : function(self) {
			if(this._getAdditionalResource(self, 'asset').getCurrentRecord() != null){
				return this._getAdditionalResource(self, 'asset').getCurrentRecord().get('siteid');
			}
			else{
				return null;
			}
		...
		//Reusable method: Get OrgId of the current Asset
		_getWorkorderOrgId : function(self) {
			return this._getAdditionalResource(self, 'asset').getCurrentRecord().get('orgid');
		},
