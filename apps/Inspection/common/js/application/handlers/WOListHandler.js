...
define("application/handlers/WOListHandler", 
  ...
  function(declare, Deferred, all, ModelService, ModelData, ModelDataSet, ApplicationHandlerBase, CommunicationManager, WorkOrderTimer, WorkOrderObject, UserManager, MessageService, lang, PlatformRuntimeException, PlatformRuntimeWarning, PlatformConstants, Logger, MaxVars, CommonHandler,WOGeolocationHandler) {
	return declare(ApplicationHandlerBase, {
		...
		render : function (eventContext) {
			...
			if (eventContext.application['platform.handlers.spatial.SpatialMapHandler']) {
				this.mobileMaximoSpatial =
                eventContext.application['platform.handlers.spatial.SpatialMapHandler'].mobileMaximoSpatial;
				if (this.mobileMaximoSpatial && this.mobileMaximoSpatial != null && this.mobileMaximoSpatial.sketchTool != null) {
		...
		handleUnsavedSketches: function(eventContext) {
			...
			this.mobileMaximoSpatial = (eventContext.application['platform.handlers.spatial.SpatialMapHandler'])?
                eventContext.application['platform.handlers.spatial.SpatialMapHandler'].mobileMaximoSpatial: null;
			if (this.mobileMaximoSpatial != null && this.mobileMaximoSpatial.sketchTool != null) {
				...
				if (sketchTool.isCurrentSketchSaved == false) {	
					ModelService.save(eventContext.getResource()).then(function (dataSet) {
						dataSet['resourceID'] = 'workOrder';
...
/**@memberOf application.handlers.WOListHandler */
		...
		refreshClassDescription: function(eventContext) {
			var refreshDeferred = new Deferred();
			var wo = eventContext.getCurrentRecord();
			var classstructureid = wo.get('classstructureid');
			if (classstructureid != null) {		
				...
				ModelService.filtered('classstructure', null, filter, 10, false, true).then(function(dataSet) {
					if (dataSet.count() > 0) {
						...
						wo.set('classificationdesc', classRec.get('description'));	
						wo.set('classificationpath', classRec.get('hierarchypath'));
					} else {
						wo.set('classificationdesc', wo.get('classstructureid'));	
						wo.set('classificationpath', wo.get('classstructureid'));
					}					
					refreshDeferred.resolve(dataSet);
				}).otherwise(function(e){
					Logger.trace('error fetching classstructure info');
