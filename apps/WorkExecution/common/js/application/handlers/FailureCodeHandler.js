...
/**@memberOf application.handlers.FailureCodeHandler */
		...
		populateCurrentFCLevel : function(eventContext){ 
			...
			if (wo && wo != null) {
				...
				if (wo.failureReportlist && wo.failureReportlist.data && wo.failureReportlist.data.length > 0){
					try{
				...
				console.log("WO not available to populateCurrentFCLevel");
				deferred.reject();
			}
			...
			if (wo.get("failurecode")==null){
				...
				promise.then(function(nextleveldoesexist){
					if (nextleveldoesexist.count()>0){
						...
						eventContext.ui.getCurrentViewControl().baseWidget.scrollTo(0,0);
					}else if (!wo.isNew()){
		...
		hideShowSelectLink : function(eventContext){
			...
			promise.then(function(failureListSet){
				...
				var checkFailureCodeHierarchy = false;
				if (failureListSet.data.length == 0 && wo.failureReportlist.data.length == 0) {
					checkFailureCodeHierarchy = true; // Initial load when CLASS is yet to be selected
				} else if (failureListSet.data.length > 0 && wo.failureReportlist.data.length == 0) {
					checkFailureCodeHierarchy = true; // Here CLASS is selected however PROBLEM , CAUSE & REMEDY or further levels is yet to be selected
				} else if (failureListSet.data.length > 0 && wo.failureReportlist.data.length > 0) {
					for (let i = 0; i < wo.failureReportlist.data.length; i++) {
						if (failureListSet.data[i] && failureListSet.data[i].type == wo.failureReportlist.data[i].type) {
							checkFailureCodeHierarchy = false;
							break;
						} else if (failureListSet.data[i].type != wo.failureReportlist.data[i].type) {
							checkFailureCodeHierarchy = true; // Here multiple levels are selected yet not complete
							break;
						}
					}
				} else if (failureListSet.data.length == 0 && wo.failureReportlist.data.length > 0) {
					checkFailureCodeHierarchy = false; // The end of Hierarchy is reached
				}

				if (checkFailureCodeHierarchy){
					var type;
					if (failureListSet.data.length > 0 && failureListSet.data[0].type){
		...
		resolveNextFailureType : function(eventContext) {
			...
			if (failureListSet && failureListSet.data && failureListSet.data[0] && failureListSet.data[0].type){
				type = failureListSet.data[0].type;
			} else {
