...
function(declare, formatterService, ModelService, UserManager, CommonHandler, ApplicationHandlerBase, PlatformRuntimeException, PlatformRuntimeWarning, PlatformConstants, SynonymDomain, MultiLabelStateMachineSupport, WorkOrderStatusHandler, Logger) {
...
/**@memberOf application.handlers.TaskHandler */
		...
		onHide: function(eventContext) {
			var self = this;
			self.application.showBusy();
			var woSet = self.application.getResource('workOrder');
			ModelService.save(woSet)
			.then(function(savedWOSet) {
				if(savedWOSet) {
					var wo = savedWOSet.getCurrentRecord();
					if(wo) {
						wo.set("statusdesc", WorkOrderStatusHandler.getInstance().toDescription(wo.get("status")));
						wo.set("internalStatus", WorkOrderStatusHandler.getInstance().toInternalState(wo.get("status")));
					}
				}
				self.application.hideBusy();
			})
			.otherwise(function(error) {
				Logger.errorJSON('[TaskHandler.onHide] ModelDataSet "workOrder" could not be saved: ', error);
				self.application.hideBusy();
			});
		}
