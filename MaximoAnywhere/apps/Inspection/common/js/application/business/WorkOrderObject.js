...
/**@memberOf application.business.WorkOrderObject */
		...
		canRemoveRecord: function(record) {
			return record.json["started"];
		},
		...
		changeStatusOfTasks: function(workOrder, newStatus, statusDate, memo, taskSet){
			...
			array.forEach(inheritingTasks, function(record){
				if(WorkOrderStatusHandler.getInstance().canPerformTransition(record.get("status"), newStatus) && (record.Status != record.offlinestatus && record.offlinestatus != "COMP")){
