...
/**@memberOf application.handlers.ReportDowntimeHandler */
		...
		_defaultValues: function(isStatusChange){
			...
			var downtimeReport =  workOrder.downtimereport.getCurrentRecord();
		...
		saveDowntime: function(eventContext){
			...
			//CORRECTION OF APAR IJ35695	
			if(isChangeStatus){
				workOrder.assetup = !assetUp;
				if(!tempDowntimeResource.get('enddate')){
					workOrder.updownstatusdate = tempDowntimeResource.get('startdate');
				}else{
					workOrder.updownstatusdate = tempDowntimeResource.get('enddate');
				}			
			}
