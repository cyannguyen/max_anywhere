			...
			ModelService.save(woSet).then(function() {
				...
				assetMeterPromise.then(function(assetMeterSet){
					if ((assetMeterSet.count()>0)) {	
						...
						//APAR:IJ35965 If MeterItem is part of a group data of assetMeterSet comes in array. We need check the value insert is equal a one element in array
						if(assetMeterSet.data.length > 1){
							assetMeterSet.data.forEach((localAsset) => {
								if(localAsset.meterdesc ==  myLocalMeter.meterdesc){
									assetmeter = localAsset;
								}
							});
						}
						//END OF CORRECTION
