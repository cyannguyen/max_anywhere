...
/**@memberOf application.handlers.SRAttachmentHandler */
			...
			if (downloadCompleted) {
			    ...
			    ConnectivityChecker.checkConnectivityAvailable().
			        ...
			        if (isConnectionAvailable) {
			            ...
			            self.ui.show('ServiceRequest.DownloadAttachmentLocal');
			            self._updatePercentComplete(0);
			            canceled = false;
		...
		"displayLocal": function(eventContext) {
			...
			if (webLink){
				...
				if(currentRecord.get('fileSize')){
					currentRecord.set('displaySize',FormatterService.toDisplayableValue((currentRecord.get('fileSize')/1024), "double", userLocale) + ' ' + MessageService.createStaticMessage('kilobytes').getMessage());
				}else {
					currentRecord.set('displaySize',FormatterService.toDisplayableValue(currentRecord.get('anywhereAttachSize'), "double", userLocale) + ' ' + MessageService.createStaticMessage('kilobytes').getMessage());
				}
			}
		...
		initCategory: function(eventContext) {
			var self = this;
			this.setAttachCategoryLookUp('ServiceRequest.appDocTypeLookup', 'docType');
