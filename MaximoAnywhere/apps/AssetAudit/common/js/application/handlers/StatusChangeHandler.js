...
function(declare, array, ApplicationHandlerBase, ModelService, PlatformRuntimeException, PlatformRuntimeWarning, 
		PlatformConstants, FormatterService, SynonymDomain, Logger, AssetObject, AssetStatusHandler, CommonHandler, AADetailsHandler) {
...
/**@memberOf application.handlers.StatusChangeHandler */
		...
		commitAssetStatusChange: function(eventContext){			
			...
			if(asset.isNew()){
				...
				var assetnumMeta = eventContext.ui.application.getResource("asset").getCurrentRecord().getRuntimeFieldMetadata('assetnum');
				assetnumMeta.set('readonly',true);
				assetnumMeta.set('required',false);
			}
		...
		_saveStatusChange: function(asset){
			...
			asset.getModelDataSet('assetAudit', false).then(function(assetAudit){
				var now = new Date();
				var aADetailsHandler = new AADetailsHandler();
				aADetailsHandler.createAuditRec(assetAudit.getParent(), assetAudit, now);
			
				var now = new Date();
				
				AssetObject.changeStatus(asset, newStatus, rolltoallchildren, removefromactiveroutes, removefromactivesp, changepmstatus, now);
		...
		_filterStatus: function(record,eventContext){
			...
			var internalStatus = SynonymDomain.resolveToInternal(domainAssetStatus, currentStatus); 
			if(internalStatus)
